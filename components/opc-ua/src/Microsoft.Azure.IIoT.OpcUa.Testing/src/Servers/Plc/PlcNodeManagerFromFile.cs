    ...
    public class PlcNodeManagerFromFile : PlcNodeManager
    {
        ...
        public override void CreateAddressSpace(IDictionary<NodeId, IList<IReference>> externalReferences)
        {
            lock (Lock)
            {
                ...
                Utils.Trace($"{nameof(PlcNodeManagerFromFile)} bootstrap started.");

                Utils.Trace($"{nameof(PlcNodeManagerFromFile)} created folder {cfgFolder.Name} in namespace {cfgFolder.NamespaceId}.");

                foreach (var node in cfgFolder.NodeList)
                {
                    CreateBaseVariable(folderFromConfigFile, node);
                    Utils.Trace($"{nameof(PlcNodeManagerFromFile)} created node {node.NodeId} - {node.Name}.");
                }
                ...
                Utils.Trace($"{nameof(PlcNodeManagerFromFile)} bootstrap completed.");
                Utils.Trace($"");
            }
